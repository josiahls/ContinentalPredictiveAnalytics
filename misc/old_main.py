        ...
        return html.Div(
            [
                                ...
                                html.Div(
                                    [
                                        dcc.RangeSlider(
                                            ...
                                            step=1,
                                            marks={
                                                0: {'label': LeavingDate_min.strftime('%m/%d/%Y'),
                                                    'style': {'font-size': fontsizeS, 'color': 'white',
                                                              'font-weight': 'bold'}},
                                                LeavingDate_range: {'label': LeavingDate_max.strftime('%m/%d/%Y'),
                                                                    'style': {'font-size': fontsizeS, 'color': 'white',
                                                                              'font-weight': 'bold'}},
                                            },
                                    ...
                                    style={'margin-left': '30', 'margin-right': '60', 'font-size': fontsizeS,
                                           'font-weight': 'bold'},
                                ),
                            ],
                                ...
                                html.Div(
                                    [
                                        dcc.RangeSlider(
                                            ...
                                            step=1,
                                            marks={
                                                int(age_min): {'label': str(age_min),
                                                               'style': {'font-size': fontsizeS, 'color': 'white',
                                                                         'font-weight': 'bold'}},
                                                int(age_max): {'label': str(age_max),
                                                               'style': {'font-size': fontsizeS, 'color': 'white',
                                                                         'font-weight': 'bold'}},
                                            },
                                        ),
                ...
                html.Div(
                    ...
                    className='row',
                    style={'margin-top': '40', 'margin-left': '40', 'font-size': fontsizeS,
        ...
        return html.Div(
            ...
            [
                ...
                html.Div(
                    [
                        dcc.Markdown(
                            ''' [Data Source](https://www.kaggle.com/HRAnalyticRepository/employee-attrition-data/data)'''),
                    ],
                    style={
                        'text-align': 'center'
                    },
...
def update_LeavingDate_text(LeavingDate_slider):
    ...
    return "Filter by leaving Date: {} to {}".format(LeavingDate_disp1.strftime('%m/%d/%y'),
                                                     LeavingDate_disp2.strftime('%m/%d/%y'))
...
def filter_dataframe(TerminationData, Age_slider, Gender_selector, LeavingDate_slider, Esubgroup_selector,
                     Esubgroup_dropdown):
...
def update_NoofData_text(Age_slider, Gender_selector, LeavingDate_slider, Esubgroup_selector, Esubgroup_dropdown):
    dff = filter_dataframe(TerminationData, Age_slider, Gender_selector, LeavingDate_slider, Esubgroup_selector,
                           Esubgroup_dropdown)
...
def update_TermPlot_text(main_graph_hover, main_graph_selected, Age_slider, Gender_selector,
    ...
    if main_graph_hover is None:
        ...
        locationname = custdata['Personnel Area - State'].iloc[0] + '-' + custdata['Personnel Area - City'].iloc[
            0] + '<br>Total no. of termination data:' + str(custdata.shape[0])
...
def make_individual_figure(main_graph_hover, main_graph_selected, Age_slider, Gender_selector, LeavingDate_slider,
    ...
    if ((main_graph_hover is None) | (plotallclicks != plotallclicknumber)):
        ...
        locationname = custdata['Personnel Area - State'].iloc[0] + '-' + custdata['Personnel Area - City'].iloc[
            0] + '<br>Total no. of data:' + str(custdata.shape[0])
    ...
    dfM = dfI.resample(PredSel, convention='end', on='Leaving date').sum().reset_index().sort_values(
        by='Leaving date')  # by month
    dfM = dfM.fillna(0)
    dfF = dfM
