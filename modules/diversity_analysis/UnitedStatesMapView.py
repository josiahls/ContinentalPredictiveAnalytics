...
class UnitedStatesMapView(View):
    ...
    def setFields(self, category=None, unique_values=None, date_slider=None):
        ...
        self.filtered_values = self.filtered_values.loc[self.filtered_values['Hire.Date'] < max(self.date_range)]
        self.filtered_values = self.filtered_values.loc[self.filtered_values['Hire.Date'] > min(self.date_range)]

    def get_view(self):
        # For each unique value, get the max value for normalizing
        normalized_max = pd.DataFrame(self.filtered_values).groupby(['Coordinates'])[
        ...
        for unique_value in self.unique_values:
            ...
            value_groups = pd.DataFrame(rows_for_unique_value).groupby(['Coordinates'],
            ...
            for i in range(0, value_groups[self.current_category].count()):
                ut.context(str(value_groups['Coordinates'].iloc[[i]]))
                if float(value_groups['Coordinates'].iloc[i].split(',')[1]) > 30 or \
                                float(value_groups['Coordinates'].iloc[i].split(',')[0]) < 10:
                    continue

                location = dict(
                    type='scattergeo',
                    locationmode='USA-states',
                    lon=float(value_groups['Coordinates'].iloc[i].split(',')[1]),
                    lat=float(value_groups['Coordinates'].iloc[i].split(',')[0]),
                    ...
                    name=str(str('{} '.format(value_groups[self.current_category].iloc[i])) + ' {} (s)'.format(
                        unique_value))
                )
    ...
    def set_callbacks(self, app=dash.Dash()):
        @app.callback(Output('diversity_map_graph', 'figure'), [Input('category_diversity_dropdown', 'value'),
                                                              Input('diversity_unique_value_dropdown', 'value'),
                                                              Input('date_diversity_slider', 'value')])
        def update_chart(category_diversity_dropdown, diversity_unique_value_dropdown, date_diversity_slider):
            self.setFields(category=category_diversity_dropdown,
                           unique_values=diversity_unique_value_dropdown,
                           date_slider=date_diversity_slider)
